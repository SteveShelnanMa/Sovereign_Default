...
call mpi_allgather(dar2(1),nii*S,mpi_double_precision,&
daragg2(1),nii*S,mpi_double_precision,mpi_comm_world,ierr)
...
dummy(:)=minloc(abs(zm-Ez))
iz=dummy(1)

num=dot_product(invpdfz,Vns(is,:,A))
num2=(num*(one-beta)*(one-gamma))**(one/(one-gamma))

funcreg(xx0,1)=lambinvvec(xx0)
funcreg(xx0,2)=lambda
funcreg(xx0,3)=num2
funcreg(xx0,4)=meanmeanspr
funcreg(xx0,5)=-meanmeandebt
funcreg(xx0,6)=meanmeandef

!!!!!!!!!!!!!!!!!Results!!!!!!!!!!!!!!!!!!!!

open(20,file='Table 7.out') 

Write(20,'(10(2X,A))') 'Quarters', 'Lambda', 'Cert. Eq. Cons', 'Avg. Spread', 'Avg. b/y', 'Def Freq'
do i=1,xx0
