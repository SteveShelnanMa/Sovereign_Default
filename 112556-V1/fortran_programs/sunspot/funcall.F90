...
do ia2=1,ambound(ia)
paym(ia2)=-q(iz,ia2)*(am(ia2)-(one-lambda)*am(ia))
fixnum(ia2)=zamvec(iz,ia)+paym(ia2)
end do
...
if (num2<num1) then ! if current consumption is higher for a higher debt level (or lower asset level), find the threshold s.
...
else ! In this case ia2-1 is dominated by ia3 (num2>num1_ that is consumption is higher with the choice of higher asset level),
...
!check the region for sm for which there is default if there is no new borrowing
...
do int2=intnc,srcmax
num2=paym(int2)

if (num2<=mxspt) then

num=Vdsnone(iz)-Vn_fut(iz,int2)
if (num>=0) then
cons1(int2-intnc+1)=thrmax
else
cons1(int2-intnc+1)=((one-gamma)*num)**(one/(one-gamma))-fixnum(int2)
end if
...
dummy(:)=minloc(cons1(1:srcmax-intnc+1))
defthc=cons1(dummy(1))

if (defthc<smthresh(1)) then
nochang=.true.
end if
...
if (.NOT.nochang) then
thcomb2(1)=min(defthc, smthresh(S+1))
chdef2(1)=.true.
asscomb2(1)=0
combint2=1

do int2=1,combint
if (thcomb(int2)>thcomb2(1)) then
int3=combint-int2
thcomb2(2:2+int3)=thcomb(int2:combint)
chdef2(2:2+int3)=chdef(int2:combint)
asscomb2(2:2+int3)=asscomb(int2:combint)
combint2=2+int3
exit
end if
end do
...
! utility integration without run
...
do while (i<=combint2.AND.isind<=S) 
if (i==1) then
if (chdef2(i)) then
isind=S
do j=2,S+1
if (thcomb2(1)<smthresh(j)) then
isind=j-1
exit
end if
end do

ucons2(isind)=ucons2(isind)+((thcomb2(1)-smthresh(isind))/smdiff(isind))*Vdsnone(iz)

if (isind>1) then
ucons2(1:isind-1)=Vdsnone(iz)
end if
...
if (thcomb2(i)>=smthresh(isind+1)) then

num2=sm(isind)+fixnum(asscomb2(i))
ucons2(isind)=ucons2(isind)+(num2**(1-gamma)/(1-gamma)+Vn_fut(iz,asscomb2(i)))
q_fut2(isind)=q_fut2(isind)+(lambda+(one-lambda)*(coup+q(iz,asscomb2(i))))
...
num=(thcomb2(i)-smthresh(isind))/smdiff(isind)

num2=sm(isind)+fixnum(asscomb2(i))
ucons2(isind)=ucons2(isind)+num*(num2**(1-gamma)/(1-gamma)+Vn_fut(iz,asscomb2(i)))
q_fut2(isind)=q_fut2(isind)+num*(lambda+(one-lambda)*(coup+q(iz,asscomb2(i))))
...
num=(thcomb2(i)-latval)/smdiff(isind)
num2=sm(isind)+fixnum(asscomb2(i))
ucons2(isind)=ucons2(isind)+num*(num2**(1-gamma)/(1-gamma)+Vn_fut(iz,asscomb2(i)))
q_fut2(isind)=q_fut2(isind)+num*(lambda+(one-lambda)*(coup+q(iz,asscomb2(i))))
...
qpr=(one-spt)*dot_product(pdfs,q_fut)+spt*dot_product(pdfs,q_fut2)    
uexpt=(one-spt)*dot_product(pdfs,ucons)+spt*dot_product(pdfs,ucons2)
end if
...
val2(iins)=qpr
valint(iins)=combint
valint2(iins)=combint2

iins2=(iins-1)*DA
dar(iins2+1:iins2+DA)=thcomb
darint(iins2+1:iins2+DA)=asscomb
dar2(iins2+1:iins2+DA)=thcomb2
darint2(iins2+1:iins2+DA)=asscomb2

iins2=(iins-1)*S
dsv1(iins2+1:iins2+S)=ucons
...
call mpi_allgather(valint2(1),nii,mpi_integer,&
valintagg(1),nii,mpi_integer,mpi_comm_world,ierr)
...
call mpi_allgather(dar2(1),nii*DA,mpi_double_precision,&
daragg(1),nii*DA,mpi_double_precision,mpi_comm_world,ierr)
...
call mpi_allgather(darint2(1),nii*DA,mpi_integer,&
darintagg(1),nii*DA,mpi_integer,mpi_comm_world,ierr)
...
do ia2=1,DA
	i1=i1+1	
	assfin2(iz,ia,ia2)=darintagg(i1)
end do
end do
end do

call mpi_allgather(dsv1(1),nii*S,mpi_double_precision,&
dsagg(1),nii*S,mpi_double_precision,mpi_comm_world,ierr)
...
do t=1,TB-1
 ...
 if (history(t)==1.OR.(history(t)==0.AND.shockh(t,ss)<ent)) then    ! if included in fin markets or redeemed into fin markets.	
    ...
    if (shockp(t,ss)>spt) then
	  ...
	  thcomb2=thfin2(zpath(t),apath(t),:)
        do i=1,endint2(zpath(t),apath(t))
        if (spath(t)<=thcomb2(i)) then 
        int1=i
        exit
        end if
        end do
	  
        if (spath(t)>thcomb2(endint2(zpath(t),apath(t)))) then
        int1=endint2(zpath(t),apath(t))       
        end if
	  int3=assfin2(zpath(t),apath(t),int1)
    end if 
	...
	if (int3>0) then
  	  ...
  	  apath(t+1)=int3
      csim(t)=fixnumall(zpath(t),apath(t),apath(t+1))+spath(t)  
...
rpath=(lambda+(1-lambda)*coup-lambda*qpath(1:TB-1))/qpath(1:TB-1) !interest rate
...
dummy(:)=minloc(abs(sm))
is=dummy(1)
num=dot_product(invpdfz,Vns(is,:,A))
welfc=(num*(one-beta)*(one-gamma))**(one/(one-gamma))

funcreg(kkk,1)=welfc
funcreg(kkk,2)=meanmeanspr
funcreg(kkk,3)=-meanmeandebt
...
Write(20,'(2X)') 
do i=1,kkk
 write(20,'(A,2X,f8.5)') 'Probability of Sunspot:', sptvec(i)
 write(20,'(A,2X,f8.5)') 'Certainty Equivalent Consumption:', funcreg(i,1)
 write(20,'(A,2X,f8.5)') 'Average Spread:', funcreg(i,2)
 write(20,'(A,2X,f8.5)') 'Debt-to-Y:', funcreg(i,3)
