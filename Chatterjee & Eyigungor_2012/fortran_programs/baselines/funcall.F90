...
real(8), intent(in), dimension(6)::xx0
...
open(20,file='Program Results.out') 

Write(20,'(2X)') 

Write(20,'(A)') 'Maturity structure'

Write(20,'(1(A,f8.4,2X))') 'lambda=', lambda 

Write(20,'(1(A,f8.4,2X))') 'coupon=', coup

write(20,'(2X)') 

Write(20,'(A)') 'Calibrated Parameters'

Write(20,'(1(A,f8.5,2X))') 'beta=', beta

Write(20,'(1(A,f8.5,2X))') 'd0 (default cost)=', defp0

Write(20,'(1(A,f8.5,2X))') 'd1 (default cost)=', defp1

write(20,'(2X)') 

Write(20,'(A)') 'Results'

 write(20,'(A,2X,f8.5)') 'Average Spread:', meanmeanspr
 
 write(20,'(A,2X,f8.5)') 'Std Dev of Spread:', stdn1(3)
 
 write(20,'(A,2X,f8.5)') 'Debt-to-Y:', -meanmeandebt
 
 write(20,'(A,2X,f8.5)') 'Default Frequency:', meanmeandef

 write(20,'(A,2X,f8.5)') 'stdev(c)/stdev(y):', stdn1(2)/stdn1(1)
 
 write(20,'(A,2X,f8.5)') 'stdev(NX/y)/stdev(y):', stdn1(4)/stdn1(1)
 
 write(20,'(A,2X,f8.5)') 'corr(y,c):', corr1(1)
 
 write(20,'(A,2X,f8.5)') 'corr(NX/y,y):', corr1(3)
 
 write(20,'(A,2X,f8.5)') 'corr(r-r_f,y):', corr1(2)
 
 write(20,'(A,2X,f8.5)') 'Avg. Debt Service:', meanmeansv
 
 write(20,'(A,2X,f8.5)') 'Default Frequency:', meanmeandef
 
 close(20) 

open(20,file='Error in Price and Value Function.out') !20 is the file number (can be anything)
 write(20,'(A,2X,A)') 'Error in Price', 'Error in Value Function'
do i=1,time
 write(20,'(2f8.5)') errtot_q(i), errtot_v(i)
end do
close(20)

funcmin=100*(spreaddata-meanmeanspr)**2+((-meanmeandebt)-debtdata)**2+10*(stdn1(3)-sprvoldata)**2
